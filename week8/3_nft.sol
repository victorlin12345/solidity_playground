...
contract AppWorks is ERC721, Ownable {
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if (msg.sender == owner()) {
        require(addressMintedBalance[msg.sender] + _mintAmount <= 20,"Owner limit to 20");
    } else {
        require(addressMintedBalance[msg.sender]+_mintAmount <=10,"Per user limit to 10");
    }
  ...
  function withdrawBalance() public returns (bool) {
      // TODO: check msg.sender is in merkleRoot
      require(addressMintedBalance[msg.sender] != 0, "balance is 0");
      addressMintedBalance[msg.sender] = 0;
      (bool success, ) = payable(msg.sender).call{value: addressMintedBalance[msg.sender]}("");
      return success;
  }
